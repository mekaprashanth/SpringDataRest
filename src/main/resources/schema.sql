...
CREATE SEQUENCE IF NOT EXISTS PUBLIC.SYSTEM_SEQUENCE_clientseq START WITH 1 BELONGS_TO_TABLE;    
CREATE SEQUENCE IF NOT EXISTS PUBLIC.SYSTEM_SEQUENCE_userseq START WITH 1 BELONGS_TO_TABLE;    
CREATE SEQUENCE IF NOT EXISTS PUBLIC.SYSTEM_SEQUENCE_authoritiesseq START WITH 1 BELONGS_TO_TABLE;    
...
create cached table PUBLIC.appuser(
    ID BIGINT DEFAULT (NEXT VALUE FOR PUBLIC.SYSTEM_SEQUENCE_userseq) NOT NULL NULL_TO_DEFAULT SEQUENCE PUBLIC.SYSTEM_SEQUENCE_userseq,
...
create cached table public.authorities (
    ID BIGINT DEFAULT (NEXT VALUE FOR PUBLIC.SYSTEM_SEQUENCE_authoritiesseq) NOT NULL NULL_TO_DEFAULT SEQUENCE PUBLIC.SYSTEM_SEQUENCE_authoritiesseq,
	user_Id varchar(50) not null,
	authority varchar(50) not null
);


create cached table public.client_details (
	CLIENTID BIGINT DEFAULT (NEXT VALUE FOR PUBLIC.SYSTEM_SEQUENCE_clientseq) NOT NULL NULL_TO_DEFAULT SEQUENCE PUBLIC.SYSTEM_SEQUENCE_clientseq,
	clientname varchar(50) not null,
	clientsecret varchar(256) not null,
	enabled boolean not null
);

ALTER TABLE PUBLIC.appuser ADD CONSTRAINT PUBLIC.userpk PRIMARY KEY(ID);               
ALTER TABLE PUBLIC.client_details ADD CONSTRAINT PUBLIC.clientpk PRIMARY KEY(clientid);               
ALTER TABLE PUBLIC.authorities  ADD CONSTRAINT PUBLIC.authoritiesfk FOREIGN KEY(user_Id) REFERENCES PUBLIC.appuser(id) NOCHECK;                
